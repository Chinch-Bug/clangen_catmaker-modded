        ...
        def GenSprite(phenotype, sprite_age, merle=False):
            ...
            def calculate_red_stripes(base, rufousing):
                ...
                if is_apricot and basecolour[:-1] in ["red", "honey"]:
                    basecolour = ("cream" if basecolour[:-1] == "red" else "ivory") + basecolour[-1]
                    main_ruf_block = 2
                    next_ruf_block = 0
            ...
            def create_stripes(stripecolour, whichbase, coloursurface=None, preset_pattern=None, special=None):
                ...
                is_dark_sunshine = (phenotype.wbtype not in [
                    "shaded", "chinchilla"] and phenotype.corin[0] == "sh2" and not_red
            ...
            def make_cat(whichmain, whichcolour, whichbase, cat_unders, special=None):
                ...
                if not is_red and not is_white and cat.pelt.rusting and None not in cat.pelt.rusting:
            ...
            def ApplyPatchEffects(sprite):
                ...
                (phenotype.agouti[0] != 'a' and (phenotype.corin[0] == 'sg' or phenotype.corin[0] == 'sh' or phenotype.corin[0] == 'sh2' or (phenotype.silver[0] == 'i' and phenotype.corin[0] == 'fg') or (phenotype.ext[0] == 'ea' and sprite_age > 6) or 'ec' in phenotype.ext) or (phenotype.ext[0] == 'ea' and sprite_age > 6))):
                    sunshine = pygame.Surface(
                        (sprites.size, sprites.size), pygame.HWSURFACE | pygame.SRCALPHA)
                    sunshine.blit(
                    ...
                    colours = phenotype.FindRed(
                        phenotype, sprite_age, special='nosilver' if is_bimetal else None)
            ...
            if (phenotype.white_pattern != 'No' and phenotype.white_pattern and phenotype.white[0] != "wsal"):
                for x in phenotype.white_pattern:
                    if (x and 'dorsal' not in x and not x.startswith("STRIPE_") and 'break/' not in x and x not in vitiligo):
                        whitesprite.blit(
            ...
            if phenotype.white_pattern and phenotype.white[0] != "wsal":
