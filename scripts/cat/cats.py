...
class Cat():
    ...
    def randomize_looks(self, just_pattern=False):
        ...
        self.phenotype.SetPoints(choice(['Normal', 'Colourpoint', 'Mink', 'Sepia', 'Point-Albino', 'Sepia-Albino', 'Siamocha', 'Burmocha', 'Mocha', 'Mocha-Albino']))
        self.genotype.tortiepattern = choice(list(tortie_patches_shapes.keys()))
        if self.genotype.sexgene is not ['O', 'O']:
            self.genotype.sexgene = choice([['o', 'o'], ['o', 'o'], ['O', 'o']])
            if 'O' in self.genotype.sexgene:
                self.phenotype.tortie = True
            else:
                self.phenotype.tortie = False
        self.phenotype.SetBaseColour(choice(['Black', 'Blue', 'Red', 'Cream', 'White', 'Albino', 'Chocolate', 'Lilac', 'Cinnamon', 'Fawn', 'Dove', 'Platinum', 
          'Honey', 'Ivory', 'Champagne', 'Lavender', 'Buff', 'Beige']).lower())
        self.genotype.dilutemd[0] = choice(['dm', 'dm', 'dm', 'dm', 'dm', 'Dm'])
        self.genotype.bleach[0] = choice(['Lb', 'Lb', 'Lb', 'Lb', 'Lb', 'lb'])
        self.genotype.ghosting[0] = choice(['gh', 'gh', 'gh', 'gh', 'gh', 'Gh'])
        self.genotype.satin[0] = choice(['St', 'St', 'St', 'St', 'St', 'st'])
        self.genotype.brindledbi = (random.random() < 0.1)
        if (random.random() < 0.1 and self.genotype.tortiepattern):
            self.genotype.tortiepattern = 'rev' + self.genotype.tortiepattern
        elif self.genotype.tortiepattern:
            self.genotype.tortiepattern = self.genotype.tortiepattern.replace('rev', '')

        self.genotype.karp = choice([['k', 'k'], ['k', 'k'], ['k', 'k'], ['k', 'k'], ['k', 'k'], ['k', 'k'], ['K', 'k'], ['K', 'k'], ['K', 'k'], ['K', 'K']])
        ...
        self.phenotype.refone = 'R' + str(randint(1, 11))
        self.phenotype.refext = 'R' + str(randint(1, 11))
        self.phenotype.pigone = choice(['P11', 'P10', 'P9', 'P8', 'P7', 'P6', 'P5', 'P4', 'P3', 'P2', 'P1', 'blue'])
        self.phenotype.pigext = choice(['P11', 'P10', 'P9', 'P8', 'P7', 'P6', 'P5', 'P4', 'P3', 'P2', 'P1', 'blue'])

        if random.random() < 0.1:
            self.phenotype.reftwo = 'R' + str(randint(1, 11))
            self.phenotype.pigtwo = choice(['P11', 'P10', 'P9', 'P8', 'P7', 'P6', 'P5', 'P4', 'P3', 'P2', 'P1', 'blue'])
        else:
            self.phenotype.reftwo = self.phenotype.refone
            if random.random() < 0.25:
                self.phenotype.pigtwo = 'blue'
            else:    
                self.phenotype.pigtwo = self.phenotype.pigone
        self.phenotype.UpdateEyes()

        if random.random() < 0.1:
            self.genotype.extraeye = 'sectoral' + str(randint(1, 6))
        else:
            self.genotype.extraeye = None
        ...
        tabbies = {"agouti" : "Agouti", "redbarc" : "Reduced Ticked (Classic)", "redbar" : "Reduced Ticked", "fullbarc" : "Ticked (Classic)", 
                  ...
                  "rosetted" : "Rosetted"}
        self.phenotype.SetTabbyPattern(choice(list(tabbies.keys())))

        self.phenotype.SetTabbyType(choice(['Solid', 'Solid', 'Solid', 'Agouti', 'Agouti', 'Agouti', 'Midnight Charcoal', 'Twilight Charcoal']))

        self.genotype.soktype = choice(['normal markings', 'normal markings', 'normal markings', 'normal markings','normal markings' , 'full sokoke', 'mild fading', 'mild fading'])

        self.genotype.silver[0] = choice(['I', 'i', 'i'])
        self.genotype.wbtype = choice(['low', 'medium', 'high', 'shaded', 'chinchilla'])
        self.genotype.ruftype = choice(['low', 'medium', 'rufoused'])

        self.genotype.ext[0] = choice(['E', 'E', 'E', 'E', choice(['Eg', 'ea', 'ea', 'er', 'ea', 'ec'])])
        self.genotype.sunshine[0] = choice(['N', 'N', 'N', 'N', choice(['sh', 'sh', 'sg', 'fg'])])
        ...
        if self.genotype.white[0] != 'W':
            for i in range(2):
                ...
                if randint(1, 25) == 1:
                    self.genotype.white[i] = "wg"
                elif randint(1, 25) == 1:
                    self.genotype.white[i] = "wt"
                elif randint(1, 2) == 1:
                    self.genotype.white[i] = "ws"
                else:
                    self.genotype.white[i] = "w"
            
            if self.genotype.white[0] == "wg":
                self.genotype.white[0] = self.genotype.white[1]
                self.genotype.white[1] = "wg"
            elif self.genotype.white[0] == "w" and self.genotype.white[1] != "wg":
                self.genotype.white[0] = self.genotype.white[1]
                self.genotype.white[1] = "w"
            elif self.genotype.white[0] == "wt" and self.genotype.white[1] != "wg" and self.genotype.white[1] != "w":
                self.genotype.white[0] = self.genotype.white[1]
                self.genotype.white[1] = "wt"
        ...
        if len(self.genotype.white_pattern) == 0:
            self.genotype.white_pattern = [None]
        elif self.genotype.white_pattern[0] not in vitiligo:
            self.genotype.white_pattern = [None] + self.genotype.white_pattern
